...
export class ProductsController {
  constructor(private readonly productsService: ProductsService,private caslAbilityFactory : CaslAbilityFactory) {}


  @Post()
  async create(@Body('kd_prd') kd_prd:string, @Body('nm_prd') nm_prd:string, @Body('hrg_prd') hrg_prd:number, @Body('stok_prd') stok_prd:number,@Body('tgl_expired') tgl_expired:string,@Body('userId') userId:number) {

      const result = await this.productsService.create(kd_prd,nm_prd,hrg_prd,stok_prd,tgl_expired,userId);
